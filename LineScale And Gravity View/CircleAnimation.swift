...
import UIKit
class CircleAnimation: AnimationDelegate {
    ...
    func setUpAnimation(in layer: CALayer, size: CGSize, color: UIColor) {
        ...
        satelliteInLayer2(layer, size: size, color: color)
        satelliteInLayer3(layer, size: size, color: color)
    }
    ...
    func ring1InLayer(_ layer: CALayer, size: CGSize, color: UIColor) {
        ...
        // Draw circle
        let circle = ActivityShape.circle.layerWith(CGSize(width: coreSize, height: coreSize), color: color)
    ...
    func ring2InLayer(_ layer: CALayer, size: CGSize, color: UIColor) {
        ...
        // Draw circle
        let circle = ActivityShape.circle.layerWith(CGSize(width: coreSize, height: coreSize), color: .yellow)
        let frame = CGRect(x: (layer.bounds.size.width - coreSize) / 2,
                           y: (layer.bounds.size.height - coreSize) / 2,
                           width: coreSize,
                           height: coreSize)
    ...
    func satelliteInLayer(_ layer: CALayer, size: CGSize, color: UIColor) {
        ...
        // Draw circle
        let circle = ActivityShape.circle.layerWith(CGSize(width: satelliteSize, height: satelliteSize), color: .red)
    ...
    func satelliteInLayer2(_ layer: CALayer, size: CGSize, color: UIColor) {
        // Rotate animation
        let rotateAnimation = CAKeyframeAnimation(keyPath: "position")
        
        rotateAnimation.path = UIBezierPath(arcCenter: CGPoint(x: layer.bounds.midX, y: layer.bounds.midY),
                                            radius: (size.width - satelliteSize) / 1,
                                            startAngle: CGFloat(M_PI) * 2.0,
                                            endAngle: CGFloat(M_PI) * 2.0 + 4 * CGFloat(M_PI),
                                            clockwise: true).cgPath
        ...
        // Draw circle
        let circle = ActivityShape.circle.layerWith(CGSize(width: satelliteSize, height: satelliteSize), color: .purple)
        let frame = CGRect(x: 0, y: 0, width: satelliteSize, height: satelliteSize)
    ...
    func satelliteInLayer3(_ layer: CALayer, size: CGSize, color: UIColor) {
        // Rotate animation
        let rotateAnimation = CAKeyframeAnimation(keyPath: "position")
        
        rotateAnimation.path = UIBezierPath(arcCenter: CGPoint(x: layer.bounds.midX, y: layer.bounds.midY),
                                            radius: (size.width - satelliteSize) / 1.5,
                                            startAngle: CGFloat(M_PI) * 1.5,
                                            endAngle: CGFloat(M_PI) * 1.5 + 4 * CGFloat(M_PI),
                                            clockwise: true).cgPath
        rotateAnimation.duration = duration * 1
        rotateAnimation.repeatCount = HUGE
        rotateAnimation.isRemovedOnCompletion = false
        
        // Draw circle
        let circle = ActivityShape.circle.layerWith(CGSize(width: satelliteSize, height: satelliteSize), color: .white)
        let frame = CGRect(x: 0, y: 0, width: satelliteSize, height: satelliteSize)
